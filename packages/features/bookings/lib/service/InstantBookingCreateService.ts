...
export interface InstantBookingMeta {
...
export async function handler(
  ...
  // Emit booking audit event (fire-and-forget — must not break the booking flow)
  try {
    ...
    const bookerAttendee = newBooking.attendees?.find(
      (attendee) => attendee.email === bookerEmail
    );
    const auditActor = getBookingAuditActorForNewBooking({
      bookerAttendeeId: bookerAttendee?.id ?? null,
    ...
    await deps.bookingEventHandler.onBookingCreated({
      ...
      auditData: buildBookingCreatedAuditData({
        booking: {
          ...
          endTime: newBooking.endTime,
          status: newBooking.status as BookingStatus,
          userUuid: null, // Instant bookings have no assigned host initially
        },
        attendeeSeatId: null,
      }),
      source: actionSource,
    ...
    logger.error("Error while firing booking audit event for instant booking", safeStringify(error));
  }
