...
export async function handler(
  ...
  bookingMeta?: { userUuid?: string; impersonatedByUserUuid?: string }
) {
  ...
  // Fire booking audit event (fire-and-forget)
  try {
    ...
    const bookerAttendeeId =
      newBooking.attendees?.find((attendee) => attendee.email === bookerEmail)?.id ?? null;

    const actor = getBookingAuditActorForNewBooking({
      bookerAttendeeId,
    ...
    const source = getAuditActionSource({
    ...
    logger.error("Failed to emit booking audit event for instant booking", safeStringify(error));
  }
