...
const mockGetOrgIdFromMemberOrTeamId = vi.fn();
vi.mock("@calcom/lib/getOrgIdFromMemberOrTeamId", () => ({
  __esModule: true,
  default: (...args: unknown[]) => mockGetOrgIdFromMemberOrTeamId(...args),
}));

const mockCheckIfTeamHasFeature = vi.fn().mockResolvedValue(false);
vi.mock("@calcom/features/flags/features.repository", () => {
  return {
    FeaturesRepository: class MockFeaturesRepository {
      checkIfTeamHasFeature(...args: unknown[]) {
        return mockCheckIfTeamHasFeature(...args);
      }
    },
  };
});
...
describe("handleInstantMeeting", () => {
  ...
  describe("team event instant meeting", () => {
    ...
    it("should emit booking audit event when booking-audit feature flag is enabled", async () => {
      ...
      // Mock org resolution and feature flag check
      mockGetOrgIdFromMemberOrTeamId.mockResolvedValue(100);
