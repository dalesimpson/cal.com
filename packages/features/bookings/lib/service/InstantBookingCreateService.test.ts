...
describe("handleInstantMeeting", () => {
  ...
  describe("booking audit events", () => {
    let onBookingCreatedSpy: ReturnType<typeof vi.spyOn>;

    beforeEach(() => {
      onBookingCreatedSpy = vi
    ...
    it("should fire booking audit event with correct data when org has booking-audit feature", async () => {
      ...
      expect(callArgs.actor).toHaveProperty("identifiedBy");
      ...
      expect(typeof callArgs.source).toBe("string");
    });

    it("should not throw when booking audit event fails", async () => {
      onBookingCreatedSpy.mockRejectedValue(new Error("Audit service unavailable"));
